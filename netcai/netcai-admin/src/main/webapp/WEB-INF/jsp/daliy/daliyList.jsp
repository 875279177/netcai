...
<body>
  ...
  <div class="content-wrapper">
    ...
    <!-- Main content -->
    <shiro:hasPermission name="daliy:query">
	    <section class="content">
	      <div class="row">
	        <div class="col-xs-12">
	          <div class="box">
	            <div class="box-header">
	              <h3 class="box-title">销售日报</h3>
	            </div>
	            <!-- 表单start -->
	            <form id="searchForm" class="form-horizontal" method="post" action="/admin/daliy/list" >
	                ...
	                <tbody>
		               <c:forEach items="${paginator.object}" var="r"> 
			   			 <tr>
			   			   ...
			   			   <td>${r.saleName}</td>
			   			   <td>${r.task1}</td>
					       <td <c:if test="${r.taskReal1<r.task1}">style="color: red;"</c:if>>${r.taskReal1}</td>
	                       <td>${r.task2}</td>
					       <td <c:if test="${r.taskReal2<r.task2}">style="color: red;"</c:if>>${r.taskReal2}</td>
					       <td>${r.sdStreet}</td>
					       <td><fmt:formatDate value="${r.sdTime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
					       <td>
...
$(function(){
 $("#reset").click(function(){
	   window.location.href="/admin/daliy/list";
 });
 loadsaleId();
});
var saleId = '${saleId}';
...
function exportExcel(){
	window.location.href="/admin/daliy/export?"+$('#searchForm').serialize();
}
</script>
</body>
</html>