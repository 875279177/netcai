       ...
       <div class="layui-inline">
	     <div class="layui-form-item">
		   ...
		   <div class="layui-input-inline">
		     <select id="province" name="provinceId"  lay-filter="province" lay-verify="required" >
                 <option value="1" selected>湖北省</option>
		     </select>
		   </div>
		   <label class="layui-form-label">市：</label>
		   <div class="layui-input-inline">
		     <select id="city" name="cityId" lay-filter="city" lay-verify="required" >
		       <option value="2" selected>武汉市</option>
		     </select>
		   </div>
	   ...
	   <div class="layui-inline">
	     ...
	     <div class="layui-input-inline">
		   <select id="seller" name="sellerId" lay-filter="seller" lay-verify="required" >
		     <option value="" selected>请选择商铺</option>
		   </select>
		 </div>
		 <label class="layui-form-label">商品分类：</label>
	     <div class="layui-input-inline">
		   <select id="category" name="categoryIds" multiple="multiple" lay-filter="category" lay-verify="required" >
		     <option value="" selected>请选择商品分类</option>
		     <option value="1" selected>商品分类1</option>
		     <option value="2" selected>商品分类2</option>
		     <option value="3" selected>商品分类3</option>
		   </select>
		 </div>
		 <label class="layui-form-label">抽点率：</label>
	     <div class="layui-input-inline">
	       <input type="text" name="percentage" placeholder="请输入抽点率" value="${percentageConfig.percentage }" lay-verify="required" class="layui-input" >
	     </div>
	   </div>
...
layui.use(['form', 'jquery', 'layedit', 'laydate'], function(){
	  ...
	  form.on('select(seller)', function(data){
          $.getJSON("<%=request.getContextPath()%>/admin/percentageConfig/getCategorys?sellerId="+data.value, function(data){
              var optionstring = "";
              $.each(data.object, function(i,item){
                  optionstring += "<option value=\"" + item.categoryId + "\" >" + item.categoryName + "</option>";
              });
	  ...
	  form.on('submit(addBuyer)', function(data){
		  ...
		  var requestData={
							  ...
							  "categoryIds":data.field.categoryIds,
							  "percentage":data.field.percentage
			              };
	      $.ajax({
	    	  type:"POST",
	    	  url:"<%=request.getContextPath()%>/admin/percentageConfig/insert",
	    	  data:requestData,
	    	  async:false,
